defmodule Ohno.Builder.GetStagesJob do
  import ShorterMaps

  import Ohno.BaseSchema,
    only: [
      load_schema: 1,
      errors_to_map: 2
    ]
  ...
  defp validate_stage(pipeline, stages, {name, item}) do
    ...
    if not (stage_group in stages) do
      ...
      errors = %{"ohno/#{name}/stage" => message}
  ...
  @ohno_schema load_schema(:ohno)
  defp validate_manifest(manifest) do
    with {:error, errors} <- Validator.validate(@ohno_schema, manifest) do
      {:error, {:invalid_schema, errors_to_map("ohno", errors)}}
    end
  end
