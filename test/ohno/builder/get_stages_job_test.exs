defmodule Ohno.Builder.GetStagesJobTest do
  ...
  alias Ohno.Builder.{Pipeline, Stage}
  alias Ohno.Builder.GetStagesJob, as: Job
  ...
  describe "extract stages from pipeline" do
    ...
    test "get ohno.yaml from repo and create stages", ~M{pipeline} do
      {:ok, %{"./ohno.yaml" => manifest}} = mock_adapter(pipeline, @good_case)
    ...
    test "make job for stage after create stages", ~M{pipeline} do
      ...
      %Stage{jobs: [%Ohno.Builder.Job{id: job_id}]} = stage
    ...
    test "valid yaml and save errors in pipeline", ~M{pipeline} do
      ...
      assert {"ohno", "Required property stages was not present."} in errors
      assert {"ohno/test", "Required property config was not present."} in errors
      assert {"ohno/test/invalid_key", "Schema does not allow additional properties."} in errors
      assert {"ohno/test/stage", "Type mismatch. Expected String but got Integer."} in errors
    end
    ...
    test "validate stages groups before create stage", ~M{pipeline} do
      ...
      assert {"ohno/release/stage", "Stage group release is missing in stages."} in errors
    end

    def mock_adapter(pipeline, content) when is_bitstring(content) do
      mock_adapter(pipeline, %{"./ohno.yaml" => content})
    end
